...
export class PositionMemberRepositoryOrm implements IPositionMemberRepository {
  ...
  async findPositionMember(
    user: any,
  ): Promise<PositionMember[]> {
    return await this.posionMemberRepository.find({
      where: {
        user: {
          id: user.id,
        },
  ...
  async create(
    entity: Partial<PositionMemberM>,
    manager?: any,
  ): Promise<PositionMemberM> {
  ...
  async removeAll(position: any, manager: EntityManager): Promise<void> {
    await Promise.all(
      position.map(async (id: string) => {
        const technicalProject = await this.findById(id);
        await this.posionMemberRepository.remove(technicalProject);
      }),
    );
  }
}